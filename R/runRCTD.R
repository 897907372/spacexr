...
process_beads_batch <- function(cell_type_info, gene_list, puck, class_df = NULL, constrain = T,
                                MAX_CORES = 4, MIN.CHANGE = 0.001, CONFIDENCE_THRESHOLD = 10, DOUBLET_THRESHOLD = 25) {
  beads = t(as.matrix(puck@counts[gene_list,]))
...
process_beads_multi <- function(cell_type_info, gene_list, puck, class_df = NULL, constrain = T,
                                MAX_CORES = 4, MIN.CHANGE = 0.001, MAX.TYPES = 4, CONFIDENCE_THRESHOLD = 10, DOUBLET_THRESHOLD = 25) {
  beads = t(as.matrix(puck@counts[gene_list,]))
  if(MAX_CORES > 1) {
    numCores = parallel::detectCores();
    if(parallel::detectCores() > MAX_CORES)
...
decompose_batch <- function(nUMI, cell_type_means, beads, gene_list, constrain = T, OLS = F, max_cores = 4, MIN.CHANGE = 0.001) {
